...
# Видаляємо пропуски у відсотковій ставці
df_scatter = df.dropna(subset=['loan_int_rate'])
...
sns.scatterplot(
    ...
    x='loan_amnt',
    y='loan_int_rate',
    hue='loan_status',  # 0 або 1 (сплачено / не сплачено)
    palette='coolwarm',
    alpha=0.6  # прозорість для кращої видимості
)

plt.title('Графік розсіювання: Сума кредиту vs. Відсоткова ставка')
